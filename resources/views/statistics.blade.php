<x-layout class="max-w-5xl w-full mx-auto space-y-6" :title="__('user.statistics.title')">
    ...
    <div class="grid grid-cols-1 gap-3 md:gap-4 xl:gap-6">
        <x-ui.card variant="glass">
            ...
            <x-ui.card.content class="p-4 pt-6 space-y-8 gap-1 flex items-center">
                <table class="table-auto md:table-fixed w-full">
                    <tbody class="divide-y">
                        <tr>
                            <td class="py-2">{{ __('user.statistics.stats.clips-submitted.stats.total') }}</td>
                        ...
                        <tr>
                            <td class="py-2">{{ __('user.statistics.stats.clips-submitted.stats.week') }}</td>
                            <td class="py-2 flex gap-2 items-center">
                                <x-lucide-video defer class="size-3 sm:size-4 md:size-6"/>
                                {{ Number::abbreviate($clipsSubmittedWeek ?? 0, maxPrecision: 1) }}
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <x-ui.card variant="glass">
            ...
            <x-ui.card.content class="p-4 pt-6 space-y-8">
                <table class="table-auto md:table-fixed w-full">
                    <tbody class="divide-y">
                        <tr>
                            <td class="py-2">{{ __('user.statistics.stats.votes.stats.total') }}</td>
                            <td class="py-2 flex gap-2 items-center">
                        ...
                        <tr>
                            <td class="py-2">{{ __('user.statistics.stats.votes.stats.week') }}</td>
                            <td class="py-2 flex gap-2 items-center">
                                <x-lucide-heart defer class="size-3 sm:size-4 md:size-6"/>
                                {{ Number::abbreviate($votesWeek ?? 0, maxPrecision: 1) }}
                            </td>
                        </tr>
                        <tr>
                            <td class="py-2">{{ __('user.statistics.stats.votes.stats.30days') }}</td>
                            <td class="py-2 flex gap-2 items-center">
                                <x-lucide-heart defer class="size-3 sm:size-4 md:size-6"/>
